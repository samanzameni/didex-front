...
export interface IFormatter {
  ...
  parse?(
    value: string
  ): ErrorFormatterParseResult | SuccessFormatterParseResult;
}
...
export interface IWatchedValueReadonlySpawn<T>
...
export interface IWatchedValueSpawn<T>
  extends IWatchedValueReadonlySpawn<T>,
    IWatchedValue<T> {
  spawn(): IWatchedValueSpawn<T>;
}
...
export interface IBrokerConnectionAdapterHost {
  ...
  showOrderDialog<T extends PreOrder>(
    ...
    handler: (
      order: T,
      customFieldsResult?: CustomInputFieldsValues
    ) => Promise<void>,
    focus?: OrderTicketFocusControl,
  ...
  showPositionBracketsDialog(
    ...
    brackets: Brackets,
    focus: OrderTicketFocusControl | null,
    handler: (brackets: Brackets) => void
  ): Promise<void>;
  showNotification(
    title: string,
    text: string,
...
export interface IChartWidgetApi {
  ...
  createStudy<
    TStudyInputs extends StudyInputValueType,
  ...
  createPositionLine(
    options: CreateTradingPrimitiveOptions
